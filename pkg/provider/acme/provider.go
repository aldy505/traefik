...
type Configuration struct {
	...
	Storage              string   `description:"Storage to use. It can be a local file path, a Redis URL (redis://, rediss:// or redis+sentinel://), or an etcd URL (etcd://, etcds://, http:// or https://) to use a distributed storage. Appending a 'file' query parameter to a distributed URL (e.g. redis://localhost:6379?file=/var/traefik/acme.json) enables a local JSON file fallback: the data is then also written to the file, and reads fall back to it when the distributed storage is unreachable." json:"storage,omitempty" toml:"storage,omitempty" yaml:"storage,omitempty" export:"true"`
